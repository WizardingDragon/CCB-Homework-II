...
def change_input(filename, frac_p, frac_w, N_beads, seed=None):
    """Creates a new dmpci.pcs_sim with updated polymer bead number (cf dpd doc)"""
        ...
        with open(filename+'_sim', 'wt') as wf:
            ...
            for line in rf:
                if line.startswith('Polymer PEG'):
                    ...
                    line[2] = frac_p
                    line[3:] = '"  (B ({} B) B)  "'.format(int(N_beads))
                    # Converts list to list[str]
                    line = list(map(lambda x: str(x), line))
                    wf.write(' '.join(line[0:3])+ ' '+ ''.join(line[3:])+ '\n')
...
def main():
    ...
    with open('results.log', 'wt') as f:    
        f.write("{:<8s}\t{:<8s}\t{:<8s}\t{:<8s}\t{:<8s}\n".format("N bead", "Mean", "Std", "Seed", "FracP"))
    ...
    print(seeds)
    notif_max = N_p*seeds.shape[0]
    notif_ind = 0
    N_bead = np.linspace(10, 51, 42)
    for i in range(N_bead.shape[0]):
        for seed in seeds:
            # Polymer density
            frac_p = "%.4f" % round(N_p/(density * V_sim), 4)
            frac_w = "%.4f" % round(1 - (N_p/(density * V_sim)), 4)
            change_input('dmpci.pcs', frac_p, frac_w, N_bead[i], seed)
            ...
            with open('results.log', 'a') as f:
                f.write("{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:s}\n".format(N_bead[i], means[-1], stds[-1], seed, frac_p))
            print(N_bead[i], means[-1], stds[-1])
            notif_ind += 1
            #sub.call(['notify-send','Homework II','{}/{}'.format(notif_ind, notif_max)])
            
            # Archive simulation files
            clean_cwd('ee_polL',N_bead[i], seed)
